...
class MealRecordTest {
  ...
  void shouldRejectNullMealType() {
    assertThrows(
        NullPointerException.class, () -> MealRecord.of(null, todayAtNoon(), createDailyRecord()));
  }

  @Test
  void shouldRejectFutureDate() {
    LocalDateTime tomorrow = LocalDate.now(ARGENTINA_ZONE).plusDays(1).atTime(12, 0);
  ...
  private DailyRecord createDailyRecord() {
    ...
    return DailyRecord.of(patient, LocalDate.now(ARGENTINA_ZONE));
  }
