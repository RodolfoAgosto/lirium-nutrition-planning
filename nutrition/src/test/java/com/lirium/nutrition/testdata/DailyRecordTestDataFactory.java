...
public class DailyRecordTestDataFactory {
  ...
  @Transactional
  public DailyRecord createDailyRecord(PatientProfile patient, LocalDate date) {

    DailyRecord dailyRecord = DailyRecord.of(patient, date);
  ...
  private void createMeals(DailyRecord dailyRecord) {
    ...
    for (MealType type : MealType.values()) {
      ...
      MealRecord meal = MealRecord.of(type, defaultTime(type), dailyRecord);

      meal.addFoodPortion(chicken(), 100.0, MeasureUnit.GRAM);
