...
class NutritionPlanControllerIT extends AbstractIntegrationTest {
  ...
  void setup() {
    ...
    User emptyPatient =
        new User(
            "empty@test.com", passwordEncoder.encode("1234"), "Empty", "Patient", Role.PATIENT);
    ...
    emptyProfile.updateNutritionProfile(
        Height.of(175), Weight.of(70000), ActivityLevel.MODERATE, GoalType.WEIGHT_LOSS);
    ...
    User patient =
        new User(
            "patient@test.com", passwordEncoder.encode("1234"), "Patient", "Test", Role.PATIENT);
  ...
  void shouldReturnNutritionPlanWhenAdminRequestsById() throws Exception {
    ...
    mockMvc
        .perform(get("/api/nutrition-plans/" + nutritionPlanId).header("Authorization", adminToken))
  ...
  void shouldReturnNutritionPlanWhenOwnerPatientRequestsById() throws Exception {
    ...
    mockMvc
        .perform(
            get("/api/nutrition-plans/" + nutritionPlanId).header("Authorization", patientToken))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.id").value(nutritionPlanId));
  }
  ...
  void shouldReturnForbiddenWhenPatientRequestsAnotherPatientsNutritionPlan() throws Exception {

    NutritionPlan otherPlan = nutritionPlanTestDataFactory.createActivePlan(otherPatientProfile);

    mockMvc
        .perform(
            get("/api/nutrition-plans/" + otherPlan.getId()).header("Authorization", patientToken))
  ...
  void shouldReturnUnauthorizedWhenUserIsNotAuthenticated() throws Exception {
    ...
    mockMvc
        .perform(get("/api/nutrition-plans/" + nutritionPlanId))
  ...
  void shouldReturnNotFoundWhenNutritionPlanDoesNotExist() throws Exception {
    ...
    mockMvc
        .perform(get("/api/nutrition-plans/999999").header("Authorization", adminToken))
  ...
  void shouldReturnForbiddenWhenPatientRequestsAnotherPatientsNutritionPlans() throws Exception {
  ...
  void shouldReturnNotFoundWhenTemplateDoesNotExist() throws Exception {
    ...
    mockMvc
        .perform(
            post("/api/nutrition-plans/generate-from-template/" + emptyPatientId + "/999999")
                .header("Authorization", adminToken))
        .andExpect(status().isNotFound());
  }
