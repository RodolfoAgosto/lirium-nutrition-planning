...
class PlanMealControllerTest {
  ...
  @MockBean private PlanMealService planMealService;
  ...
  void getById_shouldReturnMeal() throws Exception {
    ...
    when(planMealService.getById(1L)).thenReturn(mealResponse());

    mockMvc
        .perform(get("/api/plan-meals/1"))
  ...
  void getByPlanDay_shouldReturnMeals() throws Exception {
    ...
    when(planMealService.getByPlanDay(1L)).thenReturn(List.of(mealSummary()));

    mockMvc
        .perform(get("/api/plan-meals/day/1"))
        .andExpect(status().isOk())
  ...
  void create_shouldReturnCreatedMeal() throws Exception {
    ...
    when(planMealService.create(any(PlanMealCreateRequestDTO.class))).thenReturn(mealResponse());
  ...
  void delete_shouldReturnOk() throws Exception {
    ...
    mockMvc.perform(delete("/api/plan-meals/1")).andExpect(status().isNoContent());
  ...
  void addPortion_shouldReturnUpdatedMeal() throws Exception {
    ...
    FoodPortionAddRequestDTO request = new FoodPortionAddRequestDTO(100L, 150.0, MeasureUnit.GRAM);
    ...
    mockMvc
        .perform(
            post("/api/plan-meals/1/portions")
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(request)))
    ...
    verify(planMealService).addPortion(eq(1L), any(FoodPortionAddRequestDTO.class));
  }

  @Test
  void removePortion_shouldReturnUpdatedMeal() throws Exception {

    when(planMealService.removePortion(1L, 2L)).thenReturn(mealResponse());

    mockMvc
        .perform(delete("/api/plan-meals/1/portions/2"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.id").value(1))
        .andExpect(jsonPath("$.type").value("BREAKFAST"));

    verify(planMealService).removePortion(1L, 2L);
  }
  ...
  void updateQuantity_shouldReturnUpdatedMeal() throws Exception {
    ...
    when(planMealService.updateQuantity(
            eq(1L), eq(2L), any(PlanFoodPortionUpdateQuantityRequestDTO.class)))
        .thenReturn(mealResponse());
    ...
    verify(planMealService)
        .updateQuantity(eq(1L), eq(2L), any(PlanFoodPortionUpdateQuantityRequestDTO.class));
  }

  private PlanMealSummaryDTO mealSummary() {
    return new PlanMealSummaryDTO(1L, "BREAKFAST");
  }

  private PlanMealResponseDTO mealResponse() {
    return new PlanMealResponseDTO(1L, "BREAKFAST", 1L, List.of());
  }
