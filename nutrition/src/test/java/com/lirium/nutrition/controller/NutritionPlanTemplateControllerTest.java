...
class NutritionPlanTemplateControllerTest {
  ...
  void shouldGetAllTemplates() throws Exception {
    ...
    List<NutritionPlanTemplateSummaryDTO> response =
        List.of(new NutritionPlanTemplateSummaryDTO(1L, "Weight Loss", GoalType.WEIGHT_LOSS));

    when(nutritionPlanTemplateService.getAll()).thenReturn(response);

    mockMvc
        .perform(get("/api/nutrition-plan-templates"))
  ...
  void shouldGetTemplateById() throws Exception {
    ...
    NutritionPlanTemplateResponseDTO response =
        new NutritionPlanTemplateResponseDTO(
            1L, "Weight Loss", "Template for fat loss", GoalType.WEIGHT_LOSS, 40, 40, 20, Set.of());

    when(nutritionPlanTemplateService.getById(1L)).thenReturn(response);

    mockMvc
        .perform(get("/api/nutrition-plan-templates/1"))
        .andExpect(status().isOk())
  ...
  void shouldCreateTemplate() throws Exception {
    ...
    NutritionPlanTemplateCreateRequestDTO request =
        new NutritionPlanTemplateCreateRequestDTO(
            "Weight Loss", "Template for fat loss", GoalType.WEIGHT_LOSS, 40, 40, 20, Set.of());
    ...
    verify(nutritionPlanTemplateService).create(any(NutritionPlanTemplateCreateRequestDTO.class));
  }
  ...
  void shouldDeleteTemplate() throws Exception {
    ...
    mockMvc.perform(delete("/api/nutrition-plan-templates/1")).andExpect(status().isNoContent());
  ...
  void shouldUpdateTemplate() throws Exception {
    ...
    NutritionPlanTemplateUpdateRequestDTO request =
        new NutritionPlanTemplateUpdateRequestDTO(
            "Updated Template", "Updated Description", GoalType.WEIGHT_LOSS, 35, 45, 20, Set.of());
    ...
    when(nutritionPlanTemplateService.update(
            eq(1L), any(NutritionPlanTemplateUpdateRequestDTO.class)))
        .thenReturn(response);

    mockMvc
        .perform(
            patch("/api/nutrition-plan-templates/1") // <-- Cambiado de put() a patch()
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(request)))
        .andExpect(status().isOk())
    ...
    verify(nutritionPlanTemplateService)
        .update(eq(1L), any(NutritionPlanTemplateUpdateRequestDTO.class));
  }
}
