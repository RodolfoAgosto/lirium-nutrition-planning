...
class PlanFoodPortionControllerTest {
  ...
  void shouldGetFoodPortionsByMeal() throws Exception {
    ...
    List<PlanFoodPortionResponseDTO> response =
        List.of(
            new PlanFoodPortionResponseDTO(1L, 10L, 100L, "Apple", 150.0, MeasureUnit.GRAM),
            new PlanFoodPortionResponseDTO(2L, 10L, 101L, "Banana", 200.0, MeasureUnit.GRAM));

    when(service.getByPlanMeal(10L)).thenReturn(response);

    mockMvc
        .perform(get("/api/plan-food-portions/meal/10"))
  ...
  void shouldGetFoodPortionById() throws Exception {
    ...
    mockMvc
        .perform(get("/api/plan-food-portions/1"))
        .andExpect(status().isOk())
