...
class PatientControllerIT extends AbstractIntegrationTest {
  ...
  void setup() {
    ...
    patient =
        userRepository.save(
            new User(
                "patient@test.com", passwordEncoder.encode("1234"), "Juan", "Perez", Role.PATIENT));

    otherPatient =
        userRepository.save(
            new User(
                "other@test.com", passwordEncoder.encode("1234"), "Pedro", "Gomez", Role.PATIENT));
  ...
  void shouldSearchPatientsAsNutritionist() throws Exception {
    ...
    mockMvc
        .perform(get("/api/patients/search").header("Authorization", nutritionistToken))
        .andExpect(status().isOk())
  ...
  void shouldGetOwnPatientDetail() throws Exception {
    ...
    mockMvc
        .perform(get("/api/patients/{id}", patient.getId()).header("Authorization", patientToken))
        .andExpect(status().isOk())
  ...
  void shouldAllowAdminToGetPatientDetail() throws Exception {
    ...
    mockMvc
        .perform(get("/api/patients/{id}", patient.getId()).header("Authorization", adminToken))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.email").value("patient@test.com"));
  }
  ...
  void shouldRejectPatientAccessToOtherPatient() throws Exception {
    ...
    mockMvc
        .perform(
            get("/api/patients/{id}", otherPatient.getId()).header("Authorization", patientToken))
