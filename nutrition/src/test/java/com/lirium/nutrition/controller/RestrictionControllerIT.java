...
class RestrictionControllerIT extends AbstractIntegrationTest {
  ...
  void shouldGetAllRestrictions() throws Exception {
    ...
    mockMvc
        .perform(get("/api/restrictions").header("Authorization", adminToken))
  ...
  void shouldGetRestrictionById() throws Exception {
    ...
    mockMvc
        .perform(
            get("/api/restrictions/{id}", restriction.getId()).header("Authorization", adminToken))
        .andExpect(status().isOk())
  ...
  void shouldUpdateRestriction() throws Exception {
    ...
    mockMvc
        .perform(
            put("/api/restrictions/{id}", restriction.getId())
                .header("Authorization", adminToken)
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    """
  ...
  void shouldReturn404WhenRestrictionDoesNotExist() throws Exception {
    ...
    mockMvc
        .perform(get("/api/restrictions/{id}", 99999L).header("Authorization", adminToken))
