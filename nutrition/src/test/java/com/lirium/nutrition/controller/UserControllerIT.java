...
class UserControllerIT extends AbstractIntegrationTest {
  ...
  void setup() {
    ...
    admin =
        userRepository.save(
            new User(
                "admin@test.com", passwordEncoder.encode("12345678"), "Admin", "Test", Role.ADMIN));
  ...
  void shouldFindUserByIdWhenAdminRequests() throws Exception {
    ...
    mockMvc
        .perform(get("/api/users/" + patient.getId()).header("Authorization", adminToken))
  ...
  void shouldReturnNotFoundWhenFindingUnknownUser() throws Exception {
    ...
    mockMvc
        .perform(get("/api/users/999999").header("Authorization", adminToken))
  ...
  void shouldFindAllUsersWhenAdminRequests() throws Exception {
    ...
    mockMvc
        .perform(get("/api/users").header("Authorization", adminToken))
        .andExpect(status().isOk())
  ...
  void shouldUpdateBasicInfoWhenAdminRequests() throws Exception {
    ...
    UserUpdateRequestDTO request =
        new UserUpdateRequestDTO("Juan Carlos", "Gonzalez", LocalDate.of(1985, 6, 15));
  ...
  void shouldReturnNotFoundWhenUpdatingUnknownUser() throws Exception {
    ...
    UserUpdateRequestDTO request =
        new UserUpdateRequestDTO("Juan", "Perez", LocalDate.of(1990, 1, 1));
  ...
  void shouldUpdateBasicInfoWhenNutritionistRequests() throws Exception {
    ...
    UserUpdateRequestDTO request =
        new UserUpdateRequestDTO("Nuevo", "Apellido", LocalDate.of(1995, 5, 5));
  ...
  void shouldReturnNotFoundWhenValidatingUnknownUser() throws Exception {
    ...
    mockMvc
        .perform(patch("/api/users/999999/validate-email").header("Authorization", adminToken))
        .andExpect(status().isNotFound());
  }
  ...
  void shouldDeleteUserWhenAdminRequests() throws Exception {
    ...
    mockMvc
        .perform(delete("/api/users/" + patient.getId()).header("Authorization", adminToken))
  ...
  void shouldReturnNotFoundWhenDeletingUnknownUser() throws Exception {
    ...
    mockMvc
        .perform(delete("/api/users/999999").header("Authorization", adminToken))
        .andExpect(status().isNotFound());
  }
  ...
  void shouldReturnBadRequestWhenDeletingDisabledUser() throws Exception {
    ...
    mockMvc
        .perform(delete("/api/users/" + disabled.getId()).header("Authorization", adminToken))
        .andExpect(status().isForbidden());
  }
  ...
  void shouldReturnForbiddenWhenNutritionistDeletesUser() throws Exception {
    ...
    mockMvc
        .perform(delete("/api/users/" + patient.getId()).header("Authorization", nutritionistToken))
        .andExpect(status().isForbidden());
  }
  ...
  void shouldReturnForbiddenWhenPatientDeletesUser() throws Exception {
    ...
    mockMvc
        .perform(delete("/api/users/" + patient.getId()).header("Authorization", patientToken))
        .andExpect(status().isForbidden());
  }
}
