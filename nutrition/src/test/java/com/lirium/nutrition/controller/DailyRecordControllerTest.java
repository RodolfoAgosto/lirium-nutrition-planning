...
class DailyRecordControllerTest {
  ...
  void shouldReturnDailyRecordById() throws Exception {
    ...
    DailyRecordResponseDTO response = new DailyRecordResponseDTO(1L, today, List.of());

    when(dailyRecordService.getById(1L)).thenReturn(response);
  ...
  void shouldReturnPatientRecordsWhenPatientAccessesOwnRecords() throws Exception {
    ...
    mvc.perform(get("/api/daily-records/patient/1").accept(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
  ...
  void shouldUpdateMealSuccessfully() throws Exception {
    ...
    mvc.perform(
            patch("/api/daily-records/meals/1")
                .contentType(MediaType.APPLICATION_JSON)
  ...
  void shouldReturnBadRequestWhenUpdateRequestIsInvalid() throws Exception {
    ...
    MealRecordUpdateRequestDTO request = new MealRecordUpdateRequestDTO("");
  ...
  void shouldAddPortionSuccessfully() throws Exception {
    ...
    MealRecordResponseDTO response =
        new MealRecordResponseDTO(
            1L, MealType.LUNCH, false, "notes", LocalDateTime.of(2025, 1, 15, 12, 0), List.of());

    when(dailyRecordService.addPortion(eq(1L), any(FoodPortionAddRequestDTO.class)))
        .thenReturn(response);

    mvc.perform(
            post("/api/daily-records/meals/1/portions")
                .contentType(MediaType.APPLICATION_JSON)
                .content(mapper.writeValueAsString(request))
                .accept(MediaType.APPLICATION_JSON))
        .andExpect(status().isCreated())
  ...
  void shouldReturnBadRequestWhenPortionRequestIsInvalid() throws Exception {
    ...
    FoodPortionAddRequestDTO request = new FoodPortionAddRequestDTO(null, -1.0, null);
  ...
  void shouldRemovePortionSuccessfully() throws Exception {
    ...
    mvc.perform(
            delete(
                "/api/daily-records/{dailyRecordId}/meals/{mealRecordId}/portions/{portionId}",
                dailyRecordId,
                mealRecordId,
                portionId))
  ...
  void shouldReturnAdherenceReportWhenPatientAccessesOwnData() throws Exception {
    ...
    mvc.perform(
            get("/api/daily-records/patient/{patientId}/adherence", patientId)
  ...
  void shouldReturnNutritionComparisonWhenPatientAccessesOwnData() throws Exception {
    ...
    mvc.perform(
            get("/api/daily-records/patient/{patientId}/nutrition-comparison", patientId)
                .param("from", from.toString())
                .param("to", to.toString())
                .accept(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
