...
class PatientControllerTest {
  ...
  void shouldSearchPatients() throws Exception {
    ...
    List<PatientSummaryDTO> response =
        List.of(new PatientSummaryDTO(1L, "Juan", "Perez", "juan@test.com", "12345678"));

    when(patientService.searchPatients("Juan", null, null, null)).thenReturn(response);

    mockMvc
        .perform(get("/api/patients/search").param("firstName", "Juan"))
    ...
    verify(patientService).searchPatients("Juan", null, null, null);
  }
  ...
  void shouldGetPatient() throws Exception {
    ...
    when(patientService.getPatientDetail(1L)).thenReturn(response);
    ...
    verify(patientService).getPatientDetail(1L);
  }
  ...
  void shouldUpdatePatient() throws Exception {
    ...
    when(patientService.updatePatient(eq(1L), any(PatientUpdateRequestDTO.class)))
        .thenReturn(response);
    ...
    verify(patientService).updatePatient(eq(1L), any(PatientUpdateRequestDTO.class));
  }
}
