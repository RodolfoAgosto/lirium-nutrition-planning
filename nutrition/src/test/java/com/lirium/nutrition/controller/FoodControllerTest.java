...
class FoodControllerTest {
  ...
  void shouldFindAllFoods() throws Exception {
    ...
    Set<FoodSummaryDTO> response = Set.of(new FoodSummaryDTO(1L, "Chicken Breast"));
  ...
  void shouldFindFoodById() throws Exception {
    ...
    FoodResponseDTO response =
        new FoodResponseDTO(
            1L, "Chicken Breast", 165, 31, 0, 4, FoodCategory.PROTEIN, Set.of(), Set.of());

    when(foodService.findById(1L)).thenReturn(response);

    mockMvc
        .perform(get("/api/foods/1"))
  ...
  void shouldCreateFood() throws Exception {
    ...
    FoodSummaryDTO response = new FoodSummaryDTO(1L, "Chicken Breast");

    when(foodService.create(any(FoodCreateRequestDTO.class))).thenReturn(response);
  ...
  void shouldUpdateFood() throws Exception {
    ...
    FoodUpdateRequestDTO dto =
        new FoodUpdateRequestDTO("Chicken", 100, 20, 0, 5, Set.of(FoodTag.GLUTEN));

    FoodSummaryDTO response = new FoodSummaryDTO(1L, "Updated Chicken");

    when(foodService.update(eq(1L), any(FoodUpdateRequestDTO.class))).thenReturn(response);

    mockMvc
        .perform(
            patch("/api/foods/1") // <-- Cambiado de put(...) a patch(...)
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(dto)))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.name").value("Updated Chicken"));

    verify(foodService).update(eq(1L), any(FoodUpdateRequestDTO.class));
  }
  ...
  void shouldDeleteFood() throws Exception {
    ...
    mockMvc
        .perform(
            delete("/api/foods/{id}", foodId) // <-- Corregido sin /v1
                .contentType(MediaType.APPLICATION_JSON))
