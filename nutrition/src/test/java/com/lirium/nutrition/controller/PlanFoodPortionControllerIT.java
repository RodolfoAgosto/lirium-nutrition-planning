...
class PlanFoodPortionControllerIT extends AbstractIntegrationTest {
  ...
  void setup() {
    ...
    meal = PlanMeal.of(MealType.LUNCH, dailyPlan);

    meal = planMealRepository.saveAndFlush(meal);

    Food chicken =
        foodRepository.save(
            Food.of("Chicken", 165, 31, 0, 4, FoodCategory.PROTEIN, EnumSet.allOf(MealType.class)));

    portion = PlanFoodPortion.of(meal, chicken, 200D, MeasureUnit.GRAM);
  ...
  void shouldGetByMeal() throws Exception {
    ...
    mockMvc
        .perform(
            get("/api/plan-food-portions/meal/{id}", meal.getId())
  ...
  void shouldGetById() throws Exception {
    ...
    mockMvc
        .perform(
            get("/api/plan-food-portions/{id}", portion.getId())
                .header("Authorization", adminToken))
        .andExpect(status().isOk())
  ...
  void shouldReturnNotFoundWhenPortionDoesNotExist() throws Exception {
    ...
    mockMvc
        .perform(get("/api/plan-food-portions/{id}", 999999L).header("Authorization", adminToken))
  ...
  void shouldReturnEmptyListWhenMealHasNoPortions() throws Exception {
    ...
    mockMvc
        .perform(
            get("/api/plan-food-portions/meal/{id}", 999999L).header("Authorization", adminToken))
        .andExpect(status().isOk());
  }
}
