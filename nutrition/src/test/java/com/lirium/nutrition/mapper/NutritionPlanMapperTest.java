...
class NutritionPlanMapperTest {
  ...
  void shouldMapNutritionPlanToDetail() {
    ...
    PlanMeal lunch = PlanMeal.of(MealType.LUNCH, monday);

    PlanFoodPortion portion = PlanFoodPortion.of(lunch, chicken, 150.0, MeasureUnit.GRAM);

    lunch.addFoodPortion(portion);
    monday.addMeal(lunch);
    plan.addDailyPlan(monday);

    NutritionPlanDetailDTO dto = NutritionPlanMapper.toDetail(plan);
