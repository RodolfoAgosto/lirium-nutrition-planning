...
class DailyRecordMapperTest {
  ...
  void shouldMapDailyRecordToResponse() {
    ...
    DailyRecord dailyRecord = DailyRecord.of(patient, LocalDate.of(2026, 6, 30));

    MealRecord meal =
        MealRecord.of(MealType.BREAKFAST, LocalDateTime.of(2026, 6, 30, 8, 0), dailyRecord);

    Food food = Food.of("Rice", 130, 3, 28, 1, FoodCategory.CARB, Set.of(MealType.BREAKFAST));

    meal.addFoodPortion(food, 200.0, MeasureUnit.GRAM);

    dailyRecord.addMeal(meal);

    DailyRecordResponseDTO dto = DailyRecordMapper.toResponse(dailyRecord);
    ...
    assertThat(mealDto.eatenAt()).isEqualTo(LocalDateTime.of(2026, 6, 30, 8, 0));

    assertThat(mealDto.portions()).hasSize(1);

    FoodPortionRecordResponseDTO portion = mealDto.portions().getFirst();
