...
class PatientProfileHistoryMapperTest {
  ...
  void toResponseDTO_shouldMapCorrectly() {
    ...
    profile.updateNutritionProfile(height, weight, ActivityLevel.ACTIVE, GoalType.WEIGHT_LOSS);

    PatientProfileHistory history = new PatientProfileHistory(profile);

    // Act
    PatientProfileHistoryResponseDTO dto = PatientProfileHistoryMapper.toResponseDTO(history);
  ...
  void toResponseDTO_shouldHandleEmptyRestrictions() {
    ...
    profile.updateNutritionProfile(
        new Height(180), new Weight(70000), ActivityLevel.ACTIVE, GoalType.WEIGHT_MAINTENANCE);
  ...
  void toSummaryDTO_shouldMapCorrectly() {
    ...
    profile.updateNutritionProfile(
        new Height(175), new Weight(80000), ActivityLevel.SEDENTARY, GoalType.MUSCLE_GAIN);

    PatientProfileHistory history = new PatientProfileHistory(profile);

    PatientProfileHistorySummaryDTO dto = PatientProfileHistoryMapper.toSummaryDTO(history);
