...
class DailyPlanRepositoryIntegrationIT {
  ...
  @Autowired private DailyPlanRepository repository;
  ...
  void shouldFindDailyPlansByNutritionPlan() {
    ...
    assertThat(result).hasSize(2).containsExactlyInAnyOrder(monday, tuesday);
  }
  ...
  void shouldFindDailyPlanByNutritionPlanAndDayOfWeek() {
    ...
    Optional<DailyPlan> result = repository.findByNutritionPlanAndDayOfWeek(plan, DayOfWeek.MONDAY);

    assertThat(result).isPresent().contains(monday);
  }
  ...
  private NutritionPlan persistNutritionPlan() {
    ...
    NutritionPlan plan =
        NutritionPlan.generate(GoalType.WEIGHT_MAINTENANCE, 2000, 150, 250, 60, profile);
  ...
  private DailyPlan persistDailyPlan(NutritionPlan plan, DayOfWeek dayOfWeek) {
