...
class NutritionPlanTemplateRepositoryIT {
  ...
  @Autowired private NutritionPlanTemplateRepository repository;
  ...
  void shouldFindByName() {
    ...
    NutritionPlanTemplate template =
        createTemplate("Low Carb", GoalType.WEIGHT_LOSS, Set.of(FoodTag.HONEY));

    em.flush();
    em.clear();

    Optional<NutritionPlanTemplate> result = repository.findByName("Low Carb");
  ...
  void shouldCheckExistsByName() {
    ...
    createTemplate("Keto", GoalType.WEIGHT_LOSS, Set.of(FoodTag.HONEY));
  ...
  void shouldFindByTargetGoal() {
    ...
    List<NutritionPlanTemplate> result = repository.findByTargetGoal(GoalType.MUSCLE_GAIN);
  ...
  void shouldFindByExcludedTagsContains() {
    ...
    NutritionPlanTemplate template =
        createTemplate("Template 1", GoalType.WEIGHT_LOSS, Set.of(FoodTag.HONEY, FoodTag.GLUTEN));

    createTemplate("Template 2", GoalType.MUSCLE_GAIN, Set.of(FoodTag.SOY));

    em.flush();
    em.clear();

    List<NutritionPlanTemplate> result = repository.findByExcludedTagsContains(FoodTag.GLUTEN);
  ...
  void shouldDeleteByName() {
    ...
    NutritionPlanTemplate keep = createTemplate("Keep", GoalType.MUSCLE_GAIN, Set.of(FoodTag.SOY));
  ...
  private NutritionPlanTemplate createTemplate(
      String name, GoalType goal, Set<FoodTag> excludedTags) {

    NutritionPlanTemplate template =
        NutritionPlanTemplate.of(name, "description", goal, 30, 40, 30, excludedTags);
