...
class FoodPortionRecordRepositoryIT {
  ...
  @Autowired private FoodPortionRecordRepository repository;
  ...
  void shouldFindByMeal() {
    ...
    List<FoodPortionRecord> result = repository.findByMeal(meal);

    assertThat(result).hasSize(1);
    assertThat(result.get(0).getMeal().getId()).isEqualTo(meal.getId());
  }
}
