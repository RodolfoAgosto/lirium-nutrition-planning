...
class PlanMealRepositoryIT {
  ...
  @Autowired private PlanMealRepository repository;
  ...
  void shouldFindMealsByDailyPlan() {
    ...
    List<PlanMeal> result = repository.findByDailyPlan(monday);

    assertThat(result).containsExactlyInAnyOrder(breakfast, lunch);
  }
  ...
  void shouldFindMealsByDailyPlanId() {
    ...
    PlanMeal breakfast = createMeal(dailyPlan, MealType.BREAKFAST);

    PlanMeal lunch = createMeal(dailyPlan, MealType.LUNCH);

    em.flush();
    em.clear();

    List<PlanMeal> result = repository.findByDailyPlanId(dailyPlan.getId());
  ...
  void shouldFindMealsByDailyPlanAndType() {
    ...
    PlanMeal expected = createMeal(dailyPlan, MealType.LUNCH);
    ...
    List<PlanMeal> result = repository.findByDailyPlanAndType(dailyPlan, MealType.LUNCH);
  ...
  void shouldReturnEmptyListWhenNoMealsMatch() {
    ...
    List<PlanMeal> result = repository.findByDailyPlanAndType(dailyPlan, MealType.DINNER);
  ...
  private PlanMeal createMeal(DailyPlan dailyPlan, MealType type) {
