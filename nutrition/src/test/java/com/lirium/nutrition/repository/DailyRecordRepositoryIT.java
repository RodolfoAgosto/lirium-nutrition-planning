...
class DailyRecordRepositoryIT {
  ...
  private User createPatientUser(String email) {
    User user = new User(email, "123", "Juan", "Perez", Role.PATIENT);
    em.persist(user);
    return user;
  }
  ...
  void shouldFindByPatientIdAndDate() {
    ...
    Optional<DailyRecord> result =
        repository.findByPatient_IdAndDate(patient.getId(), LocalDate.of(2026, 6, 30));
  ...
  void shouldReturnRecordsOrderedByDateDesc() {
    ...
    List<DailyRecord> result = repository.findByPatient_IdOrderByDateDesc(patient.getId());
  ...
  void shouldFindBetweenDates() {
    ...
    List<DailyRecord> result =
        repository.findByPatient_IdAndDateBetween(
            patient.getId(), LocalDate.of(2026, 6, 21), LocalDate.of(2026, 6, 23));
  ...
  void shouldFindWithMealsFetched() {
    ...
    List<DailyRecord> result =
        repository.findByPatient_IdAndDateBetweenWithMeals(
            patient.getId(), LocalDate.of(2026, 6, 29), LocalDate.of(2026, 6, 30));
  ...
  void shouldFindByMealId() {
    ...
    Optional<DailyRecord> result = repository.findByMealRecordId(meal.getId());
