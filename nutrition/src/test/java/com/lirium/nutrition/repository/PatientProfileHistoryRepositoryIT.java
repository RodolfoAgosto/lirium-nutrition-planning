...
class PatientProfileHistoryRepositoryIT {
  ...
  @Autowired private PatientProfileHistoryRepository repository;
  ...
  void shouldFindByPatientProfileAndVisitDateBetween() {
    ...
    List<PatientProfileHistory> result =
        repository.findByPatientProfileAndVisitDateBetween(
            patient, LocalDate.of(2026, 1, 2), LocalDate.of(2026, 1, 8));
  ...
  void shouldFindByPatientProfileAndPrimaryGoal() {
    ...
    List<PatientProfileHistory> result =
        repository.findByPatientProfileAndPrimaryGoal(patient, GoalType.MUSCLE_GAIN);
  ...
  private PatientProfile createPatient() {
    ...
    User user =
        new User("patient@test.com", UUID.randomUUID().toString(), "Juan", "Perez", Role.PATIENT);
  ...
  private PatientProfileHistory createHistory(PatientProfile patient, LocalDate visitDate) {
  ...
  private PatientProfileHistory createHistoryWithGoal(PatientProfile patient, GoalType goal) {
