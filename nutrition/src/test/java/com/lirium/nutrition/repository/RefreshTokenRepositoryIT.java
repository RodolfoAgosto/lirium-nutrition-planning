...
class RefreshTokenRepositoryIT {
  ...
  void shouldFindByUser() {
    ...
    RefreshToken expected = createRefreshToken(user, "token-1");
  ...
  void shouldFindByToken() {
    ...
    RefreshToken expected = createRefreshToken(user, "token-2");
  ...
  void shouldReturnEmptyWhenTokenDoesNotExist() {
    ...
    Optional<RefreshToken> result = repository.findByToken("unknown-token");
  ...
  private RefreshToken createRefreshToken(User user, String token) {
    ...
    RefreshToken refreshToken =
        new RefreshToken(user, token, Instant.now().plus(Duration.ofDays(30)));
