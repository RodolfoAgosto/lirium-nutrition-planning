...
class FoodRepositoryIT {
  ...
  void shouldExistsByName() {
    ...
    createFood("Banana", Set.of(MealType.BREAKFAST), Set.of());
  ...
  void shouldFindFoodsByTag() {
    ...
    Food bread = createFood("Bread", Set.of(MealType.BREAKFAST), Set.of(FoodTag.GLUTEN));

    createFood("Milk", Set.of(MealType.BREAKFAST), Set.of(FoodTag.LACTOSE));

    em.flush();

    Set<Food> result = repository.findByFoodTagsIn(Set.of(FoodTag.GLUTEN));

    assertThat(result).containsExactly(bread);
  }
  ...
  void shouldFindSuitableFoods() {
    ...
    createFood("Bread", Set.of(MealType.LUNCH), Set.of(FoodTag.GLUTEN));
