...
class PatientProfileServiceImplTest {
  ...
  @InjectMocks private PatientProfileServiceImpl patientProfileService;
  ...
  void shouldThrowWhenPatientProfileNotFound() {
    ...
    verify(patientProfileRepository).findByUserIdFetchUser(1L);
  ...
  void shouldReturnPatientProfileWhenFound() {
    ...
    // When
    PatientProfile result = patientProfileService.findByUserId(1L);
  ...
  void shouldUpdatePatientProfileSuccessfully() {
    ...
    // Given
    User user = new User("test@test.com", "password", "Rodolfo", "Agosto", Role.PATIENT);
    ...
    when(patientProfileRepository.save(patientProfile)).thenReturn(patientProfile);

    // When
    PatientProfileResponseDTO result = patientProfileService.update(patientProfile);

    // Then
    assertNotNull(result);

    verify(patientProfileRepository).save(patientProfile);
  }
}
