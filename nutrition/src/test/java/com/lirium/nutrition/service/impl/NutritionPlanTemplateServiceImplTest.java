...
class NutritionPlanTemplateServiceImplTest {
  ...
  @Mock private NutritionPlanTemplateRepository repository;

  @InjectMocks private NutritionPlanTemplateServiceImpl service;
  ...
  void shouldCreateTemplate() {
    ...
    NutritionPlanTemplateCreateRequestDTO request =
        new NutritionPlanTemplateCreateRequestDTO(
            "Weight Loss", "Template for weight loss", GoalType.WEIGHT_LOSS, 30, 40, 30, Set.of());

    NutritionPlanTemplate template =
        NutritionPlanTemplate.of(
            "Weight Loss", "Template for weight loss", GoalType.WEIGHT_LOSS, 30, 40, 30, Set.of());

    when(repository.existsByName("Weight Loss")).thenReturn(false);

    when(repository.save(any(NutritionPlanTemplate.class))).thenReturn(template);

    // When
    NutritionPlanTemplateResponseDTO result = service.create(request);
  ...
  void shouldThrowWhenCreatingDuplicateTemplate() {
    ...
    when(repository.existsByName("Weight Loss")).thenReturn(true);

    // When + Then
    DuplicateTemplateException ex =
        assertThrows(DuplicateTemplateException.class, () -> service.create(request));

    assertTrue(ex.getMessage().contains("Template already exists"));
  ...
  void shouldUpdateTemplateWithMacros() {
    ...
    NutritionPlanTemplate template =
        NutritionPlanTemplate.of(
            "Weight Loss", "Template", GoalType.WEIGHT_LOSS, 30, 40, 30, Set.of());
    ...
    // When
    NutritionPlanTemplateResponseDTO result = service.update(1L, request);
  ...
  void shouldUpdateTemplateWithoutMacros() {
    ...
    NutritionPlanTemplate template =
        NutritionPlanTemplate.of(
            "Weight Loss", "Original Description", GoalType.WEIGHT_LOSS, 30, 40, 30, Set.of());
  ...
  void shouldThrowWhenOnlySomeMacroPercentagesAreProvided() {
    ...
    assertEquals("All macro percentages must be provided together", ex.getMessage());
  ...
  void shouldThrowWhenUpdatingDuplicateName() {
    ...
    NutritionPlanTemplateUpdateRequestDTO request =
        new NutritionPlanTemplateUpdateRequestDTO(
            "Keto", "Updated Description", GoalType.WEIGHT_MAINTENANCE, null, null, null, Set.of());

    when(repository.findById(1L)).thenReturn(Optional.of(template));

    when(repository.existsByName("Keto")).thenReturn(true);

    // When + Then
    assertThrows(DuplicateTemplateException.class, () -> service.update(1L, request));
  ...
  void shouldReturnTemplateById() {
    ...
    // When
    NutritionPlanTemplateResponseDTO result = service.getById(1L);
  ...
  void shouldReturnAllTemplates() {
    ...
    NutritionPlanTemplate template1 =
        NutritionPlanTemplate.of(
            "Weight Loss", "Template 1", GoalType.WEIGHT_LOSS, 30, 40, 30, Set.of());

    NutritionPlanTemplate template2 =
        NutritionPlanTemplate.of(
            "Muscle Gain", "Template 2", GoalType.MUSCLE_GAIN, 25, 50, 25, Set.of());

    when(repository.findAll()).thenReturn(List.of(template1, template2));

    // When
    List<NutritionPlanTemplateSummaryDTO> result = service.getAll();
