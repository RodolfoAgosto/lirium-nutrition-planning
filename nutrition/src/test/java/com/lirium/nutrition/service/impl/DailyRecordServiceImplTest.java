...
class DailyRecordServiceImplTest {
  ...
  @Mock PatientProfileService patientProfileService;
  @Mock NutritionPlanService nutritionPlanService;
  @Mock FoodService foodService;
  @Mock private PatientProfileRepository patientProfileRepository;
  @Mock MealRecordRepository mealRecordRepository;
  @Mock private Clock clock;

  @InjectMocks DailyRecordServiceImpl service;
  ...
  void shouldThrowWhenCreatingDailyRecordForFutureDate() {
    ...
    assertEquals("Cannot create daily records for future dates", ex.getMessage());
  ...
  void shouldThrowWhenDateIsBeforeActivePlanStartDate() {
    ...
    when(dailyRecordRepository.findByPatient_IdAndDate(patientId, targetDate))
        .thenReturn(Optional.empty());

    when(patientProfileService.findById(patientId)).thenReturn(patient);

    when(nutritionPlanService.findActivePlan(patientId)).thenReturn(Optional.of(plan));

    when(plan.getStartDate()).thenReturn(planStart);
  ...
  void shouldThrowWhenDateRangeIsInvalid() {
    ...
    IllegalArgumentException ex =
        assertThrows(
            IllegalArgumentException.class,
            () -> service.getNutritionComparison(patientId, END, START));

    assertEquals("The 'from' date cannot be after the 'to' date", ex.getMessage());
  ...
  void shouldThrowWhenFromDateIsNull() {
    ...
    IllegalArgumentException ex =
        assertThrows(
            IllegalArgumentException.class,
            () -> service.getNutritionComparison(patientId, null, END));

    assertEquals("Date range is required", ex.getMessage());
  ...
  void shouldThrowWhenToDateIsNull() {
    ...
    IllegalArgumentException ex =
        assertThrows(
            IllegalArgumentException.class,
            () -> service.getNutritionComparison(patientId, START, null));
  ...
  void shouldAdjustEffectiveFromToPlanStartDate() {
    ...
    when(dailyRecordRepository.findByPatient_IdAndDateBetween(patientId, planStart, to))
        .thenReturn(List.of());
    ...
    verify(patientProfileRepository).existsById(patientId);

    verify(dailyRecordRepository).findByPatient_IdAndDateBetween(patientId, planStart, to);
  }
  ...
  void shouldRoundAdherenceToOneDecimalPlace() {
    ...
    when(portion.calories()).thenReturn(new Calories(1234));

    when(portion.protein()).thenReturn(new Protein(50));

    when(portion.carbs()).thenReturn(new Carbs(100));

    when(portion.fat()).thenReturn(new Fat(20));

    when(nutritionPlanService.findActivePlan(patientId)).thenReturn(Optional.of(plan));

    when(dailyRecordRepository.findByPatient_IdAndDateBetween(patientId, from, to))
  ...
  void shouldReturnTrueWhenDailyRecordBelongsToUser() {
    ...
    when(dailyRecordRepository.findById(dailyRecordId)).thenReturn(Optional.of(record));

    assertTrue(service.isDailyRecordOwnedByUser(dailyRecordId, userId));
  }
  ...
  void shouldReturnFalseWhenDailyRecordDoesNotBelongToUser() {
    ...
    assertFalse(service.isDailyRecordOwnedByUser(dailyRecordId, authenticatedUserId));
  }
  ...
  void shouldReturnFalseWhenDailyRecordDoesNotExistForOwnershipCheck() {
    ...
    assertFalse(service.isDailyRecordOwnedByUser(dailyRecordId, userId));
  }
  ...
  void shouldReturnTrueWhenMealRecordBelongsToUser() {
    ...
    when(mealRecordRepository.findById(mealRecordId)).thenReturn(Optional.of(mealRecord));

    assertTrue(service.isMealRecordOwnedByUser(mealRecordId, userId));
  }
  ...
  void shouldReturnFalseWhenMealRecordDoesNotBelongToUser() {
    ...
    assertFalse(service.isMealRecordOwnedByUser(mealRecordId, authenticatedUserId));
  }
  ...
  void shouldReturnFalseWhenMealRecordDoesNotExistForOwnershipCheck() {
    ...
    when(mealRecordRepository.findById(mealRecordId)).thenReturn(Optional.empty());

    assertFalse(service.isMealRecordOwnedByUser(mealRecordId, userId));
  }
  ...
  void shouldReturnTrueWhenDailyRecordExistsForPatientAndDate() {
    ...
    when(dailyRecordRepository.existsByPatient_IdAndDate(patientId, date)).thenReturn(true);

    assertTrue(service.existsForPatientAndDate(patientId, date));

    verify(dailyRecordRepository).existsByPatient_IdAndDate(patientId, date);
  }
  ...
  void shouldReturnFalseWhenDailyRecordDoesNotExistForPatientAndDate() {
    ...
    when(dailyRecordRepository.existsByPatient_IdAndDate(patientId, date)).thenReturn(false);

    assertFalse(service.existsForPatientAndDate(patientId, date));
  ...
  void shouldReturnDailyRecordById() {
    ...
    when(dailyRecordRepository.findById(id)).thenReturn(Optional.of(record));
    // When
  ...
  void shouldReturnEmptyListWhenNoRecords() {
    ...
    when(dailyRecordRepository.findByPatient_IdOrderByDateDesc(patientId)).thenReturn(List.of());
  ...
  void shouldCreateDailyRecordWhenNotExists() {
    ...
    when(plan.getWeek())
        .thenReturn(Collections.emptyList()); // O con DailyPlans si querés testear comidas
  ...
  void shouldThrowExceptionWhenNoActivePlan() {
    ...
    when(nutritionPlanService.findActivePlan(patientId)).thenReturn(Optional.empty());

    // When + Then
    IllegalStateException exception =
        assertThrows(
            IllegalStateException.class, () -> service.getOrCreateForDate(patientId, today));
  ...
  void shouldOverrideMealWhenNotesProvided() {
    ...
    MealRecordUpdateRequestDTO request = new MealRecordUpdateRequestDTO("Patient changed meal");
  ...
  void shouldAddFoodPortionToMeal() {
    ...
    FoodPortionAddRequestDTO request = new FoodPortionAddRequestDTO(foodId, 100D, MeasureUnit.GRAM);

    when(dailyRecordRepository.findByMealRecordId(mealId)).thenReturn(Optional.of(dailyRecord));

    when(foodService.findEntityById(foodId)).thenReturn(food);
    ...
    verify(meal).addFoodPortion(food, 100D, MeasureUnit.GRAM);
  ...
  void shouldThrowWhenMealNotFoundInAddPortion() {
    ...
    FoodPortionAddRequestDTO request = new FoodPortionAddRequestDTO(10L, 100D, MeasureUnit.GRAM);
  ...
  void shouldRemovePortionFromMeal() {
    ...
    when(activePlan.getStartDate()).thenReturn(today.minusDays(5));
  ...
  void shouldReturnZeroConsumedValuesWhenNoRecordsExist() {
    ...
    org.springframework.test.util.ReflectionTestUtils.setField(plan, "startDate", START);
    org.springframework.test.util.ReflectionTestUtils.setField(plan, "endDate", END);

    when(patientProfileRepository.existsById(patientId)).thenReturn(true);

    when(nutritionPlanService.findActivePlan(patientId)).thenReturn(Optional.of(plan));

    when(dailyRecordRepository.findByPatient_IdAndDateBetween(patientId, START, END))
        .thenReturn(List.of());

    NutritionComparisonReportDTO result = service.getNutritionComparison(patientId, START, END);
  ...
  void shouldCreateTodayRecordWithMealsFromActivePlan() {
    ...
    when(nutritionPlanService.findActivePlan(patientId)).thenReturn(Optional.of(activePlan));

    when(activePlan.getWeek()).thenReturn(List.of(dailyPlan));

    when(dailyPlan.getDayOfWeek()).thenReturn(today.getDayOfWeek());

    when(dailyPlan.getMeals()).thenReturn(List.of(planMeal));

    when(planMeal.getType()).thenReturn(MealType.BREAKFAST);
  ...
  void shouldCreateRecordWithoutMealsWhenTodayDailyPlanDoesNotExist() {
    ...
    when(otherDay.getDayOfWeek()).thenReturn(today.getDayOfWeek().plus(1));

    when(plan.getWeek()).thenReturn(List.of(otherDay));
    ...
    DailyRecord saved = captor.getValue();
    assertTrue(
        saved.getMeals().isEmpty(), "El registro debería crearse con la lista de comidas vacía");
  ...
  void shouldCreateTodayRecordWithMealsFromActivePlan(MealType mealType) {
    ...
    when(planMeal.getType()).thenReturn(mealType);
  ...
  void shouldNotOverrideMealWhenNotesAreNull() {
    ...
    when(dailyRecord.getMeals()).thenReturn(List.of(mealRecord));

    when(dailyRecordRepository.findByMealRecordId(mealRecordId))
        .thenReturn(Optional.of(dailyRecord));

    MealRecordUpdateRequestDTO request = new MealRecordUpdateRequestDTO(null);

    // When
    service.updateMeal(mealRecordId, request);

    // Then
    verify(mealRecord, never()).markAsOverridden(anyString());
  ...
  void shouldThrowWhenMealRecordNotFoundForUpdate() {
    ...
    when(dailyRecord.getMeals()).thenReturn(List.of(anotherMeal));
    ...
    verify(dailyRecordRepository).findByMealRecordId(mealRecordId);
  ...
  void shouldThrowWhenDailyRecordNotFoundForRemovePortion() {
    ...
    verify(dailyRecordRepository).findById(dailyRecordId);
  ...
  void shouldThrowWhenFoodPortionNotFoundForRemovePortion() {
    ...
    when(mealRecord.getId()).thenReturn(mealRecordId);
    when(mealRecord.getFoodPortions()).thenReturn(List.of(anotherPortion));
  ...
  private PatientProfile patientProfile() {
    ...
    patient.update(
        sex, activityLevel, Weight.of(55_000), Height.of(165), null, Set.of(), conditions, goal);

    return patient;
  }

  private NutritionPlan createPlan(int calories, int protein, int carbs, int fat) {
  ...
  private LocalDate mockClock() {
    when(clock.instant()).thenReturn(Instant.parse("2026-08-28T15:00:00Z"));

    when(clock.getZone()).thenReturn(ZoneId.of("America/Argentina/Buenos_Aires"));
