...
/**
 * Aggregate root representing a consumed meal. Contains food portions, meal type, and consumption
 * time. A meal may originate from a plan or be spontaneous (overridden).
 */
@Entity
@Getter
...
public class MealRecord extends DateAuditable {
  ...
  // Creates a record linked to a plan. Initially, 'overriden' is false as it follows the
  // prescription.
  private MealRecord(PlanMeal planMeal, LocalDateTime eatenAt, DailyRecord dailyRecord) {
    System.out.println(
        ">>> PlanMeal ID: "
            + planMeal.getId()
            + " - Cantidad de alimentos en getFoods(): "
            + (planMeal.getFoods() != null ? planMeal.getFoods().size() : "NULL"));
            ...
            food -> {
              FoodPortionRecord portion =
                  FoodPortionRecord.of(
                      this, food.getFood(), food.getQuantity(), food.getMeasureUnit());
  ...
  public static MealRecord fromPlan(
      PlanMeal planMeal, LocalDateTime eatenAt, DailyRecord dailyRecord) {
  ...
  public void addFoodPortion(Food food, Double quantity, MeasureUnit unit) {
    ...
    Optional<FoodPortionRecord> existingPortion =
        foods.stream().filter(fp -> fp.getFood().equals(food) && fp.getUnit() == unit).findFirst();
