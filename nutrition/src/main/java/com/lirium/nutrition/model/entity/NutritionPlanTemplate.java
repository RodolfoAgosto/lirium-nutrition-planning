...
/**
 * Entity representing a reusable nutrition plan template. Defines goal type, macronutrient
 * distribution and optional food tag exclusions.
 */
@Entity
@Getter
...
public class NutritionPlanTemplate extends Auditable {
  ...
  public static NutritionPlanTemplate of(
    ...
    if (excludedTags != null) template.excludedTags.addAll(excludedTags);

    return template;
  }

  public void update(
      String name, String description, GoalType targetGoal, Set<FoodTag> excludedTags) {
  ...
  private static void validateMacros(int protein, int carb, int fat) {
    ...
    if (protein + carb + fat != 100) {
      throw new UnprocessableEntityException(
          String.format(
              "Macro percentages must sum to 100. Current sum: %d", (protein + carb + fat)));
    }
  }
}
