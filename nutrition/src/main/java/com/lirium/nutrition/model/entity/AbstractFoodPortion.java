...
/**
 * Represents a quantified portion of a specific Food. This abstract base class is reused by
 * entities that need to model food quantities (e.g. meal items, recipe ingredients, historical
 * records). A portion references a Food entity and a validated amount expressed in grams.
...
public abstract class AbstractFoodPortion {
  ...
  public Fat fat() {
    return new Fat(
        (int)
            (food.getFatPer100g() * food.toGrams(this.getQuantity(), this.getMeasureUnit()) / 100));
  }
