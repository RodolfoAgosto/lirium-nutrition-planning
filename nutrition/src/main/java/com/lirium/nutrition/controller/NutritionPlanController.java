...
@Tag(
    ...
    description = "Endpoints for generating, retrieving, and managing nutritional plans.")
public class NutritionPlanController {
  ...
  @Operation(
      ...
      description =
          "Executes the calculation engine to create a tailored nutrition plan based on the patient's metrics and target goals.")
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "201",
  ...
  @Operation(
      ...
      description =
          "Activates a draft nutrition plan for the patient and automatically closes any previously active plan.")
  @SecurityRequirement(name = "bearerAuth")
  @ApiResponses(
      value = {
  ...
  @Operation(
      ...
      description =
          "Creates a new DRAFT nutrition plan for the specified patient by adapting the caloric and macronutrient distribution from a base template.")
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "201",
  ...
  public ResponseEntity<NutritionPlanDetailDTO> generateFromTemplate(
      @Parameter(description = "ID of the target patient", example = "1") @PathVariable
          Long patientId,
      @Parameter(description = "ID of the source template", example = "2") @PathVariable
          Long templateId) {

    log.info(
        "Generating nutrition plan from template templateId={} for patientId={}",
        templateId,
        patientId);
    NutritionPlanDetailDTO response =
        nutritionPlanGenerator.generateFromTemplate(patientId, templateId);
    log.info(
        "Nutrition plan generated from template templateId={} for patientId={}",
        templateId,
        patientId);
  ...
  @Operation(
      ...
      description =
          "Retrieves details of a specific nutrition plan. Accessible by ADMIN, NUTRITIONIST, or the patient who owns the plan.")
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "200",
