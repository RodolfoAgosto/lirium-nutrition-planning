...
public class AuthController {
  ...
  @Operation(
      ...
      summary = "Log in user",
      description =
          "Authenticates a user using their credentials (email and password) and returns a valid JWT token along with basic profile information.")
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "200",
  ...
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "200",
            description =
                "Token successfully refreshed. Returns new JWT access token and refresh token.",
            content =
                @Content(
                    mediaType = "application/json",
                    schema = @Schema(implementation = AuthResponseDTO.class))),
