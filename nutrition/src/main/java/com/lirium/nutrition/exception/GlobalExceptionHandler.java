...
public class GlobalExceptionHandler {
  ...
  public ResponseEntity<ApiError> handleFoodInUse(
      FoodInUseException ex, HttpServletRequest request) {
  ...
  public ResponseEntity<ApiError> handleDuplicateFood(
      DuplicateFoodException ex, HttpServletRequest request) {
  ...
  public ResponseEntity<ApiError> handleDuplicateTemplate(
      DuplicateTemplateException ex, HttpServletRequest request) {
  ...
  public ResponseEntity<ApiError> handleHandlerMethodValidation(
    ...
    log.warn(
        "Handler method validation failed path={} message={}", request.getRequestURI(), message);
  ...
  public ResponseEntity<ApiError> handleDomainValidationException(
    ...
    log.warn(
        "Domain validation error path={} message={}", request.getRequestURI(), ex.getMessage());
  ...
  public ResponseEntity<ApiError> handleUnprocessableEntity(
    ...
    log.warn(
        "Unprocessable entity error path={} message={}", request.getRequestURI(), ex.getMessage());
  ...
  public ResponseEntity<ApiError> handlePlanConflict(
      PlanConflictException ex, HttpServletRequest request) {
  ...
  public ResponseEntity<ApiError> handleHttpMessageNotReadable(
    ...
    if (ex.getCause() instanceof InvalidFormatException invalidFormatEx) {
      ...
      if (!fieldName.isEmpty()) {
        message =
            String.format(
                "Invalid value '%s' for field '%s'", invalidFormatEx.getValue(), fieldName);
      }
    } else if (ex.getCause() instanceof MismatchedInputException mismatchedInputEx) {
      String fieldName =
          mismatchedInputEx.getPath().stream()
              .map(JsonMappingException.Reference::getFieldName)
              .filter(Objects::nonNull)
              .collect(Collectors.joining("."));
  ...
  public ResponseEntity<ApiError> handleTypeMismatch(
    ...
    String message;
    // Si la propiedad destino es LocalDate (o asignable), detallamos el formato esperado ISO
    // (YYYY-MM-DD)
    if (ex.getRequiredType() != null && LocalDate.class.isAssignableFrom(ex.getRequiredType())) {
      message =
          String.format(
              "Parameter '%s' has an invalid date value '%s'. Expected format: YYYY-MM-DD",
              paramName, value);
    } else {
  ...
  public ResponseEntity<ApiError> handleRestrictionAlreadyExists(
      RestrictionAlreadyExistsException ex, HttpServletRequest request) {

    log.warn(
        "Restriction already exists path={} message={}", request.getRequestURI(), ex.getMessage());
