...
public class PlanMealServiceImpl implements PlanMealService {
  ...
  public PlanMealResponseDTO create(PlanMealCreateRequestDTO dto) {
    ...
    log.info(
        "Plan meal created successfully id={} dailyPlanId={}", saved.getId(), dto.dailyPlanId());
  ...
  public PlanMealResponseDTO addPortion(Long mealId, FoodPortionAddRequestDTO dto) {
    ...
    planMeal.addFoodPortion(PlanFoodPortionMapper.toEntity(dto, planMeal, food));
  ...
  public PlanMealResponseDTO updateQuantity(
      Long mealId, Long portionId, PlanFoodPortionUpdateQuantityRequestDTO request) {

    log.info(
        "Updating quantity to {} for portionId={} in mealId={}",
        request.quantity(),
        portionId,
        mealId);
    ...
    if (!portion.getMeal().getId().equals(mealId)) {
      ...
      throw new IllegalArgumentException(
          "Portion id=" + portionId + " does not belong to meal id=" + mealId);
    }
    ...
    log.info(
        "Portion quantity updated successfully for portionId={} in mealId={}", portionId, mealId);
