...
public class NutritionPlanTemplateServiceImpl implements NutritionPlanTemplateService {
  ...
  public List<NutritionPlanTemplateSummaryDTO> getAll() {
    ...
    List<NutritionPlanTemplateSummaryDTO> result =
        repository.findAll().stream().map(NutritionPlanTemplateMapper::toSummary).toList();
  ...
  public NutritionPlanTemplateResponseDTO create(NutritionPlanTemplateCreateRequestDTO dto) {
    ...
    NutritionPlanTemplate saved = repository.save(NutritionPlanTemplateMapper.toEntity(dto));
  ...
  public NutritionPlanTemplateResponseDTO update(
    ...
    log.debug(
        "Update payload id={} name={} goal={} macrosUpdated={}",
        id,
        dto.name(),
        dto.targetGoal(),
    ...
    if ((dto.proteinPercentage() == null) != (dto.carbPercentage() == null)
        || (dto.proteinPercentage() == null) != (dto.fatPercentage() == null)) {

      throw new IllegalArgumentException("All macro percentages must be provided together");
    }

    if (dto.proteinPercentage() != null) {
      template.updateMacros(dto.proteinPercentage(), dto.carbPercentage(), dto.fatPercentage());
    }
