...
public class MacroDistributorImpl implements MacroDistributor {
  ...
  public MacroDistribution distribute(PatientProfile patient, Calories calories) {
    ...
    log.info(
        "Distributing macros for patientId={} calories={}",
    ...
    log.info(
        "Macros distributed successfully patientId={} protein={} carbs={} fat={}",
        patient.getUser().getId(),
        protein,
        carbs,
        fat);
  ...
  @Override
  public MacroDistribution distributeFromTemplate(
      Calories calories, NutritionPlanTemplate template) {
    int totalCal = calories.amount();

    log.info(
        "Distributing macros from template templateId={} calories={}", template.getId(), totalCal);

    log.debug(
        "Template percentages protein={} carb={} fat={}",
        template.getProteinPercentage(),
        template.getCarbPercentage(),
        template.getFatPercentage());
    ...
    log.info(
        "Macros distributed from template templateId={} protein={} carbs={} fat={}",
        template.getId(),
        proteinGrams,
        carbGrams,
        fatGrams);
