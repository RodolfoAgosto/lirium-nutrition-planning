        ...
        .authorizeHttpRequests(
            auth ->
                auth
                    ...
                    // operations)
                    .requestMatchers(HttpMethod.GET, "/api/plan-food-portions/**")
                    .authenticated()
                    .requestMatchers(HttpMethod.GET, "/api/plan-meals/**")
                    .authenticated()

                    // Only ADMIN
                    .requestMatchers(HttpMethod.DELETE, "/api/users/**")
                    .hasRole(ROLE_ADMIN)
                    .requestMatchers("/api/users/*/enabled")
                    .hasRole(ROLE_ADMIN)
                    .requestMatchers(HttpMethod.DELETE, "/api/nutrition-plan-templates/**")
                    .hasRole(ROLE_ADMIN)
                    ...
                    .hasAnyRole(ROLE_ADMIN, ROLE_NUTRITIONIST)
                    .requestMatchers("/api/restrictions/**")
                    .hasAnyRole(ROLE_ADMIN, ROLE_NUTRITIONIST)
                    .requestMatchers("/api/plan-meals", "/api/plan-meals/**")
                    .hasAnyRole(ROLE_ADMIN, ROLE_NUTRITIONIST)
                    .requestMatchers("/api/nutrition-plan-templates/**")
                    .hasAnyRole(ROLE_ADMIN, ROLE_NUTRITIONIST)
                    .requestMatchers(HttpMethod.POST, "/api/nutrition-plans/**")
                    .hasAnyRole(ROLE_ADMIN, ROLE_NUTRITIONIST)
                    .requestMatchers(HttpMethod.PATCH, "/api/nutrition-plans/**")
                    .hasAnyRole(ROLE_ADMIN, ROLE_NUTRITIONIST)
                    .requestMatchers("/api/plan-food-portions/**")
                    .hasAnyRole(ROLE_ADMIN, ROLE_NUTRITIONIST)

                    // MULTI-ROLE READING (Properly authenticated in Controller/Service)
                    .requestMatchers("/api/patients/**")
                    .authenticated()
                    .requestMatchers("/api/nutrition-plans/**")
                    .authenticated()
                    .requestMatchers("/api/daily-records/**")
                    .authenticated()
                    .requestMatchers("/api/foods/**")
                    .authenticated()
                    .requestMatchers("/api/users/*/validate-email")
                    .authenticated()
                    .anyRequest()
                    .authenticated())
        .sessionManagement(
