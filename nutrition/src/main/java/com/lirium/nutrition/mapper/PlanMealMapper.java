...
public class PlanMealMapper {
  ...
  public static PlanMealResponseDTO toResponse(PlanMeal entity) {
    ...
    List<PlanFoodPortionSummaryDTO> foods =
        entity.getFoodPortions().stream().map(PlanFoodPortionMapper::toSummary).toList();

    return new PlanMealResponseDTO(
        entity.getId(), entity.getType().name(), entity.getDailyPlan().getId(), foods);
  }

  public static PlanMealSummaryDTO toSummary(PlanMeal entity) {
    return new PlanMealSummaryDTO(entity.getId(), entity.getType().name());
  }

  /* ========= CREATE DTO → ENTITY ========= */

  public static PlanMeal toEntity(PlanMealCreateRequestDTO dto, DailyPlan dailyPlan) {
  ...
  public static void updateEntity(PlanMeal entity, PlanMealUpdateRequestDTO dto) {
    if (dto.type() != null) {
