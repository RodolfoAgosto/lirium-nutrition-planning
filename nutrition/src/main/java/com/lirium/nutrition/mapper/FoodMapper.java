...
public class FoodMapper {
  ...
  public static FoodResponseDTO toResponse(Food entity) {
    ...
    Set<String> tags = entity.getFoodTags().stream().map(Enum::name).collect(Collectors.toSet());
  ...
  public static FoodSummaryDTO toSummary(Food entity) {
    return new FoodSummaryDTO(entity.getId(), entity.getName());
  }
  ...
  public static Food toEntity(FoodCreateRequestDTO dto) {
    ...
    if (dto.tags() != null) {
      dto.tags().forEach(tag -> food.addTag(tag));
    }
  ...
  public static void updateEntity(Food entity, FoodUpdateRequestDTO dto) {

    if (dto.name() != null) entity.changeName(dto.name());

    if (dto.caloriesPer100g() != null) entity.changeCalories(dto.caloriesPer100g());

    if (dto.proteinPer100g() != null) entity.changeProtein(dto.proteinPer100g());

    if (dto.carbsPer100g() != null) entity.changeCarbs(dto.carbsPer100g());

    if (dto.fatPer100g() != null) entity.changeFat(dto.fatPer100g());

    if (dto.tags() != null) {
      entity.clearTags();
      dto.tags().forEach(tag -> entity.addTag(tag));
    }
  }
}
